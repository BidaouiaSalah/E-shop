   ...
   <div class="container-fluid pt-5">
      ...
      <div class="row px-xl-5">
               ...
               @foreach ($brands as $brand)
                  <div
                     ...
                     <a href="{{ route('shop.index', ['brand' => $brand->id]) }}"
                        class="text-dark">
                        <label class="custom-control-label"
                           for="price-all">{{ $brand->name }}</label></a>
                  </div>
               @endforeach
            </div>
            <!-- Price End -->
         ...
         <div class="col-lg-9 col-md-12">
            @if ($products->count() > 0)
               <h4 class="display-5 py-3">Filtering by: {{ $filteredBy->name ?? $filteredBy }} </h4>
               <div class="row pb-3">
            ...
            @else
               <div class="row pb-3">
                  <h4 class="display-5">No Results availible for:
                     {{ $filteredBy->name ?? $filteredBy }}
                  </h4>
               </div>
            @endif
         </div>
